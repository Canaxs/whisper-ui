import { createBadge , allBadges} from "@/api/apiCalls";
...
export default function CreateBadge() {
    ...
    const [dataBackup , setDataBackup] = useState<BadgeEntity[]>([]);
    const [selectData, setSelectData] = useState<BadgeEntity[]>([]);
    ...
    const [selectBadge, setSelectBadge] = useState({ id: 0 , badge: "" , badgeURL: "" , badgeType: "" });
    ...
    function createBadgeFunction() {
            let badgeData = new FormData();
            const badgeRequest = {
                badge: badgeName
            }
            const blobImage = new Blob([iconImage as Blob], {
                type: 'multipart/form-data'
            });
            badgeData.set("badgeCreateRequest" , new Blob([JSON.stringify(badgeRequest)], {type : 'application/json'}) );
            badgeData.set("image",blobImage);
    
            createBadge(badgeData,Cookies.get("token")).then((res) => {
                toast({
                    variant: "success",
                ...
                toast({
                    variant: "destructive",
                    title: "Rozet oluşturma işlemi sırasında Hata oluştu",
        ...
        function openPopup(popupData) {
            setSelectBadge(popupData);
            setIsDialogOpen(true);
        }

        function filterBadges(str) {
            if(str === "") {
                setData(dataBackup);
                ...
                setData(data.filter((res) =>  res.badge.toLowerCase().includes(str.toLowerCase())));
    ...
    return (
        <div> 
            <h1 className="font-medium drop-shadow-sm">Tüm Rozetler</h1>
            <h6 className="text-gray-600 text-xs mt-2">Düzeltmek İstediğiniz rozet bilgisinin üzerine çift tıklayın</h6>
            <div className="mt-5 flex gap-3">
                <Input placeholder="Başlık Filtrele..." className="max-w-sm" onChange={(e) => filterBadges(e.target.value)}/>
                <Dialog>
                    ...
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle className="mt-5">Create Badge</DialogTitle>
                            <DialogDescription>
                                Badge names must be different from other data
                            </DialogDescription>
                            <div className="grid gap-4 py-4">
                                <div className="grid grid-cols-4 items-center gap-4">
                                    <Label htmlFor="name" className="text-right">Name</Label>
                                    <Input id="name" className="col-span-3" onChange={(e) => setBadgeName(e.target.value)}/>
                                </div>
                                <div className="grid grid-cols-4 items-center gap-4">
                                    <Label htmlFor="username" className="text-right">Icon Image</Label>
                                    <Input id="picture" accept='image/*' type="file" onChange={(e) => imageOnChange(e)}/>
                                </div>
                            </div>
                        </DialogHeader>
                        <DialogFooter>
                            <Button type="submit" onClick={() => createBadgeFunction()}>Save changes</Button>
                        </DialogFooter>
                    </DialogContent>
                </Dialog>
            </div>
                ...
                <div>
                    {selectData.map((obj,index) => (
                        <div key={"indexe"+index}>
                            <div className="flex justify-between cursor-pointer hover:bg-gray-300 transition-all shadow-md">
                                <div className="w-[12%] flex justify-center items-center">
                                    <Checkbox />
                                </div>
                                <span className="p-3 w-[22%] text-center">{obj.id}</span>
                                <span className="p-3 w-[22%] text-center">{obj.badge}</span>
                                <span className="p-3 w-[22%] text-center">{obj.badgeURL}</span>
                                <span className="p-3 w-[22%] text-center">{obj.badgeType}</span>
                            </div>
                        </div>
                    ))}
                </div>
