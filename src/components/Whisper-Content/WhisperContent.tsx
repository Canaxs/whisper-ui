...
export default function WhisperContent(props) {
  ...
  return (
    <div className="mt-10 w-[90%] ml-[5%]">
      <div className="flex">
        ...
        <div className="flex flex-col ml-2">
          ...
          <span className="text-xs flex">
            Oluşturulma Tarihi :{" "}
            <span className="ml-1">
              {giveTheClock(props.whisper.createdDate)} ·{" "}
            </span>{" "}
            <span className="ml-1">
              {giveTheDate(props.whisper.createdDate)}
            </span>
          </span>
        </div>
      </div>
      <div className="mt-10">
        ...
        <div className="flex">
          <div
            className={
              Cookies.get("username") != null
                ? "p-2 flex items-center hover:text-black text-gray-400 transition-all cursor-pointer"
                : "p-2 flex items-center hover:text-black text-gray-400 transition-all cursor-no-drop opacity-60"
            }
            onClick={likeExists ? () => unlike() : () => like()}
          >
            <SlLike
              className={likeExists ? "size-7 text-green-500" : "size-7"}
            />
            <span
              className={
                likeExists
                  ? "mt-[5px] font-medium ml-1 text-green-500"
                  : "mt-[5px] font-medium ml-1"
              }
            >
              {props.whisper.whisperLike.numberLike}
            </span>
          ...
          >
            <SlDislike
              className={
                dislikeExists ? "text-red-500 size-7 mt-1" : "size-7 mt-1"
              }
            />
            <span
              className={
                dislikeExists
                  ? "text-red-500 mt-[5px] font-medium ml-1"
                  : "mt-[5px] font-medium ml-1"
              }
            >
              {props.whisper.whisperLike.numberDislike}
      ...
      <div className="mt-8">
        <h1 className="text-5xl max-sm:text-xl max-md:text-2xl max-lg:text-3xl max-xl:text-4xl drop-shadow-lg font-sans">
          {props.whisper.title}
        </h1>
      ...
      <div className="mt-10">
        <div className="flex">
          ...
          >
            <SlLike
              className={likeExists ? " text-green-500 size-7" : "size-7"}
            />
            <span
              className={
                likeExists
